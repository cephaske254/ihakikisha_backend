...
class BaseAbstractModel(models.Model): 
    uuid = models.UUIDField(verbose_name='Unique User ID',)
    class Meta:
        abstract = True
...
class UserManager(BaseUserManager):
    def create_user(self,first_name,last_name,email, password, user_type='F'):
        ...
        if not first_name: raise ValueError("First Name is required")
        if not last_name: raise ValueError("Last Name is required")
